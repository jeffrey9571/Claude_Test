...
```bash
mvn compile
mvn test               # PostgreSQL 테스트 DB 필요 (아래 참고)
mvn spring-boot:run    # PostgreSQL 접속정보는 application.yml 또는 환경변수로 설정
```

테스트도 운영과 동일하게 **PostgreSQL**로 수행한다(H2 미사용). `mvn test` 전에
PostgreSQL과 테스트 DB가 준비되어 있어야 하며, Flyway가 스키마·시드 데이터를
적용한다. 기본 접속정보는 `src/test/resources/application.yml`에 있고 환경변수로
오버라이드할 수 있다.

```sql
-- 테스트 DB 준비 예시
CREATE DATABASE ifrs17_test OWNER ifrs17_app;
```

```bash
# 접속정보 오버라이드 예시
TEST_DB_URL=jdbc:postgresql://localhost:5432/ifrs17_test \
TEST_DB_USER=ifrs17_app TEST_DB_PASSWORD=*** mvn test
```
